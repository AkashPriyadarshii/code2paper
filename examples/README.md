# code2paper — Live Examples

Real repositories analyzed by the code2paper skill. Each example pairs a
`human-guide.md` (plain-language, beginners) with a `paper.*` set (formal spec, advanced).

## NEET CBT 2027 Simulator — Marketing Site

- **Repo:** https://github.com/AkashPriyadarshii/neet-cbt-site
- **Stack:** Static HTML + CSS, GitHub Pages, zero build step
- **Size:** 9 files, ~455 lines — a small repo, fully analyzed

| Artifact | Link |
| --- | --- |
| Plain-language guide | [human-guide.md](neet-cbt-site/human-guide.md) |
| Formal spec (Markdown) | [paper.md](neet-cbt-site/paper.md) |
| Interactive WebPaper | [paper.html](neet-cbt-site/paper.html) |
| Typst source | [paper.typ](neet-cbt-site/paper.typ) |
| LaTeX source | [paper.tex](neet-cbt-site/paper.tex) |
| Compiled PDF | [paper.pdf](neet-cbt-site/paper.pdf) |

What it shows: a single-file marketing site — every section grounded in the actual
`index.html` (CSS variables, JSON-LD schema, CTA flow, `<details>` FAQ), with real
measured numbers, honest limitations, full multi-format output including PDF, and an
**interactive dependency graph** in `paper.html` (drag nodes, hover to trace edges).
